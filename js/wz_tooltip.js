...
config.BgColor = '#552200' // Background colour (HTML colour value, in quotes)
...
config.FontFace = '"Roboto Slab","Helvetica Neue",Helvetica,Arial,sans-serif'
...
function tt_HideInit() {
	if (tt_iState) {
		...
		if (tt_flagOpa && tt_aV[FADEOUT]) {
			tt_tFade.EndTimer();
			if (tt_opa) {
				var n = Math.round(tt_aV[FADEOUT] / (tt_aV[FADEINTERVAL] * (tt_aV[OPACITY] /
					tt_opa)));
...
function tt_GetEvtX(e) {
	return (e ? ((typeof(e.pageX) != tt_u) ? e.pageX : (e.clientX + tt_GetScrollX())) :
		0);
}

function tt_GetEvtY(e) {
	return (e ? ((typeof(e.pageY) != tt_u) ? e.pageY : (e.clientY + tt_GetScrollY())) :
		0);
}
...
function tt_Browser() {
	...
	tt_op = (document.defaultView && typeof(eval("w" + "indow" + "." + "o" + "p" +
		"er" + "a")) != tt_u);
	...
	if (tt_ie) {
		...
		if (tt_db)
			tt_ie56 = parseFloat(nv.substring(nv.indexOf("MSIE") + 5)) >= 5.5 && typeof document
			.body.style.maxHeight == tt_u;
	} else {
		tt_db = document.documentElement || document.body ||
			(document.getElementsByTagName ? document.getElementsByTagName("body")[0] :
				null);
		if (!tt_op) {
			n6 = document.defaultView && typeof document.defaultView.getComputedStyle !=
				tt_u;
			w3c = !n6 && document.getElementById;
		}
	}
	tt_body = (document.getElementsByTagName ? document.getElementsByTagName(
...
function tt_MkMainDiv() {
	...
	else if (typeof tt_body.innerHTML != tt_u && document.createElement && tt_body
		.appendChild)
...
function tt_OpaSupport() {
	...
	tt_flagOpa = (typeof(css.KhtmlOpacity) != tt_u) ? 2 : (typeof(css.KHTMLOpacity) !=
		tt_u) ? 3 : (typeof(css.MozOpacity) != tt_u) ? 4 : (typeof(css.opacity) !=
		tt_u) ? 5 : (typeof(css.filter) != tt_u) ? 1 : 0;
}
...
function tt_HideSrcTagsRecurs(dad) {
	...
	for (var i = a ? a.length : 0; i;) {
		...
		if (!tt_HideSrcTagsRecurs(a[i]))
			return false;
		ovr = a[i].getAttribute ? (a[i].getAttribute("onmouseover") || a[i].getAttribute(
			"onclick")) : (typeof a[i].onmouseover == "function") ? (a[i].onmouseover ||
			a[i].onclick) : null;
		if (ovr) {
	...
	if (tt_aV[CLOSEBTN]) {
		...
		for (var i = 4; i;) {
			...
			if (!tt_aV[CLOSEBTNCOLORS][i].length)
				tt_aV[CLOSEBTNCOLORS][i] = (i & 1) ? tt_aV[TITLEFONTCOLOR] : tt_aV[
					TITLEBGCOLOR];
		}
...
function tt_MkTipSubDivs() {
	...
	tt_aElt[0].innerHTML =
		('' + (tt_aV[TITLE].length ?
			...
			sTbTrTd + 'id="WzBoDyI" style="' + sCss + '">' + tt_sContent +
			'</td></tr></tbody></table></div>' + (tt_aV[SHADOW] ? (
...
function tt_GetWndCliSiz(s) {
	...
	if (typeof y == sN) {
		...
		return (
			...
			// ... strict mode
			: ((db = document.documentElement) && typeof(y2 = db[sC]) == sN && y2 && y2 <=
				y) ? y2
